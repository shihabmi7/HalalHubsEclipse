...
public class MasjidListAdapter extends BaseAdapter {

	private final Context mContext;
	ArrayList<Masjid> newsItemList;
	DisplayImageOptions options;
	ImageLoader imageLoader = ImageLoader.getInstance();

	// we use universal image loader library

	public MasjidListAdapter(Context context, ArrayList<Masjid> list) {
	...
	public int getCount() {
		// return 100;
		return ApplicationData.masjidList.size();

	}

	@Override
	public Object getItem(int position) {
		return ApplicationData.masjidList.get(position);
	}
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		Masjid aMasjidObj = (Masjid) ApplicationData.masjidList.get(position);
		holder.masjid_name.setText(aMasjidObj.getName());
		holder.masjid_area.setText(aMasjidObj.getArea());

		imageLoader.displayImage(ApplicationData.masjidList.get(position)
